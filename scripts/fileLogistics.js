...
async function getFileContentsAPI(pID) {
    // Use direct fetch like updateFile does - no need to load discovery docs
    if (!gapi || !gapi.auth) {
    ...
    return await fetch('https://www.googleapis.com/drive/v3/files/' + pID + '?alt=media', {
        method: 'GET',
        headers: new Headers({ 'Authorization': 'Bearer ' + accessToken })
    }).then((res) => {
        if (res.ok) {
            return res.text().then(text => ({ body: text }));
        } else {
...
async function resurrectFails() {
    ...
    let jsonV = JSON.parse(got.result)
    //make array of file names and file contents
