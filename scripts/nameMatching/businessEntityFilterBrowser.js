/**
 * Business Entity Filter System (Browser Version)
 *
...
class BusinessEntityFilter {
    ...
    async initialize() {
        ...
        try {
            // Use same URL pattern as working legacy code (bloomerang.js)
            const reqBase = "http://127.0.0.99:3000";

            // Load complete exclusion entities
            const completeExclusionData = await this.loadCSVFile(`${reqBase}/csv-file?file=BusinessTermsMaster - NonNameFullNamesx.csv`);
            ...
            const businessTermsData = await this.loadCSVFile(`${reqBase}/csv-file?file=BusinessTermsMaster - Nonnames.csv`);
    ...
    /**
     * Load CSV file via browser fetch and return array of values
     */
